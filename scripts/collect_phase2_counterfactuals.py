#!/usr/bin/env python3
"""Collect paired factual/counterfactual transitions by real execution.

Each pair resets the same MiniWoB task with the same seed.  The factual arm
...
def _visible_alternative(state: Any, factual: ActionDecision) -> ActionDecision:
    ...
    if alternatives:
        target = sorted(alternatives, key=lambda item: (item.role, item.name, item.bid))[0]
        return ActionDecision(
            action=f'click({json.dumps(target.bid)}, "left")',
            action_type="click",
            rationale="Executed visible wrong-target counterfactual.",
            target_bid=target.bid,
            target_name=target.name,
            confidence=1.0,
        )
    if factual.action_type in {"fill", "type"} and factual.target_bid:
        return ActionDecision(
            action=f'fill({json.dumps(factual.target_bid)}, "counterfactual_wrong_value")',
            action_type="fill",
            rationale="Executed wrong-value counterfactual on the factual input.",
...
def _run_arm(
    ...
    *,
    ...
    decision = factual if role == "factual" else _visible_alternative(state, factual)
...
def main() -> int:
    ...
    for task in DEFAULT_TASKS:
        task_runs = [seed for current_task, seed in planned if current_task == task]
        if not task_runs:
            continue
        register_browsergym_environment(task)
        env = gym.make(task, headless=True)
        try:
            for seed in task_runs:
                pair_id = f"cf-{task.rsplit('.', 1)[-1]}-seed{seed}"
                try:
                    factual = _run_arm(
                        env,
                        task=task,
                    ...
                    failures.append({"task": task, "seed": seed, "error": repr(error)})
        finally:
            env.close()
