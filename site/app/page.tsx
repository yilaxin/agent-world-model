...
const issueRows = [
  ...
  ["P2", "AndroidWorld", "本机 WHPX：预检/冒烟通过（19 个 UI 元素）；最终 7 任务×5 次 reactive 91.4% vs phase3 91.4%（启动器翻页+序列导航+快速转发器恢复）；wifi 60–100%，轨迹已落盘", "扩展评测+策略增强完成"],
];
...
function Status({ value }: { value: string }) {
  const kind = value === "已解决" || value.includes("通过") || value.includes("完成") ? "ok" : value.includes("待") ? "pending" : "blocked";
  ...
  return (
    <main>
      ...
      <section className="metric-grid" aria-label="核心状态">
        ...
        <Metric label="AndroidWorld 运行时" value="API 33" note="WHPX 冒烟通过；7 任务成功率 91.4%/91.4%" />
      </section>
